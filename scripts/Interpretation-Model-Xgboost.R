...
theme_churn <- function(base_size = 13) {
  theme_minimal(base_size = base_size) +
    theme(
      ...
      plot.title       = element_text(color = COLORS$text,  size = 16, face = "bold", margin = margin(b = 6)),
      plot.subtitle    = element_text(color = COLORS$muted, size = 12, margin = margin(b = 14)),
...
save_plot <- function(name, w = 10, h = 6) {
  ggsave(paste0("plots/presentation/", name, ".png"),
         width = w, height = h, dpi = 180, bg = COLORS$bg)
  message("Sauvegardé : plots/presentation/", name, ".png")
  }
...
#=========================#
features  <- readRDS("models/xgb-model-features.rds")
xgb_model <- xgb.load("models/xgb-classifier-model.json")

x_test_mat <- as.matrix(test_data[, features])
y_test     <- as.numeric(as.character(test_data$Churn))
pred_prob  <- predict(xgb_model, xgb.DMatrix(data = x_test_mat))
...
#===============================================================#
#   SORTIE 1 — Métriques clés de performance (tableau visuel)   #
#===============================================================#
...
p_metrics <- ggplot(metrics_df,
                    aes(x = reorder(Indicateur, Valeur), y = Valeur, fill = Couleur)) +
  ...
  labs(title = "Le modèle est-il fiable ?",
       subtitle = "Performance mesurée sur des données que le modèle n'a jamais vues (jeu de test)",
       x = NULL, y = "Score",
...
p_roc <- ggplot(roc_df, aes(x = FPR, y = TPR)) +
  ...
  labs(title = "Capacité du modèle à distinguer churners et clients stables",
       subtitle = "Plus la courbe violette est haute et à gauche, meilleur est le modèle",
...
p_importance <- ggplot(imp_top10, aes(x = Feature, y = Gain, fill = Gain)) +
  ...
  labs(title = "Quelles informations client influencent le plus le modèle ?",
       subtitle = "Les variables les plus utiles pour détecter un futur churner",
       x = NULL, y = "Importance (%)",
...
#==========================================================================#
#   SORTIE 4: Comment chaque variable influence la probabilité de churn    #                               #
#==========================================================================#

shap_values <- shap.values(xgb_model = xgb_model, X_train = x_test_mat)
...
p_shap <- shap.plot.summary(shap_long) +
  ...
  labs(title = "Quel est l'impact de chaque variable sur le risque de churn ?",
       subtitle = paste0("Chaque point = 1 client  |  ",
                         "Rouge = valeur élevée de la variable | ",
                         "À droite = pousse vers le churn"),
       caption  = "Méthode SHAP : décomposition de la décision du modèle pour chaque client") +
  theme_churn() +
  theme(legend.position = "right")


print(p_shap)
save_plot("04_shap_impact_variables", w = 12, h = 12)
